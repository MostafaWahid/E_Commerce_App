...
import { db } from "../../db";
import { CheckoutSessionLine, checkoutSessions, products, users } from "../../db/schema";
...
export async function createCheckout(req: Request, res: Response, next: NextFunction) {
  ...
  const [localUser] = await db.select().from(users).where(eq(users.clerkUserId,userId)).limit(1)
    ...
    const successUrl = `${env.CLIENT_URL}/checkout/return?checkout_id={CHECKOUT_ID}`;
    const returnUrl = `${env.CLIENT_URL}/cart`;
